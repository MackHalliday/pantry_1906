...
class RecipeTest < MiniTest::Test
  ...
  def test_return_amount_of_ingredient
    @mac_and_cheese.add_ingredient(@cheese, 2)
    @mac_and_cheese.add_ingredient(@mac, 8)
  ...
  def test_return_only_ingredients
    @mac_and_cheese.add_ingredient(@cheese, 2)
    @mac_and_cheese.add_ingredient(@mac, 8)
  ...
  def test_return_total_cals_in_recipe
    @mac_and_cheese.add_ingredient(@cheese, 2)
    @mac_and_cheese.add_ingredient(@mac, 8)
  ...
  def test_total_cal_by_ingredient
    @mac_and_cheese.add_ingredient(@cheese, 2)
    @mac_and_cheese.add_ingredient(@mac, 8)

    assert_equal 200, @mac_and_cheese.total_cal_by_ingredient(@cheese)
  end

  def test_hash_ingredient_with_amount
    @mac_and_cheese.add_ingredient(@mac, 8)
    @mac_and_cheese.add_ingredient(@cheese, 2)

    expected = {:ingredient=>"Macaroni", :amount=>"8 oz"}, {:ingredient=>"Cheese", :amount=>"2 C"}

    assert_equal expected, @mac_and_cheese.each_ingredient_amount
  end
end
